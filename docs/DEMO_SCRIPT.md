...
Part B; order strictly follows charter §9. **Ready to record** once the live AWS deployment +
demo URL are up.
...
- Seg 3 (RPO=0 / RTO<10s) is **locally proven** on the 9-node simulated multi-region cluster. The
  on-camera capture uses a **real** kill on the deployed cluster; the RTO number quoted must be
...
- Seg 4 (consolidation) logic is implemented and tested locally; the live changefeed→Lambda path is
  **pending the live deploy**. If that path is not live at record time, render Seg 4 from the deterministic replay
